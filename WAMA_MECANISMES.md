...
| **Appariement d'identité de canal** | Relie une identité Matrix/Discord à un compte WAMA par code prouvé hors canal — la garde que tout adaptateur appelle avant d'agir | `wama/gateway/services.py` | `ROADMAP.md §19` | 244 |
...
| **Manifestes** | Extraction/validation/projection des 7 kinds vers les registres | `wama/common/manifests/ingest.py` | `WAMA_MANIFEST_ARCHITECTURE.md` | 64 |
...
| **Domaines → modes** | Schéma déclaratif des onglets-domaine et modes par app — scope la file | `wama/common/utils/app_modes.py` | `MODES_QUEUE_UX.md` | 17 |
...
| **Formats de téléchargement (⬇ late-binding)** | Vocabulaire commun des formats choisis AU TÉLÉCHARGEMENT (libellé, icône, groupe) + split-button dérivé de la déclaration export_binding — pendant late-binding d'output_formats ; 6ᵉ action de card | `wama/common/utils/export_formats.py` | `WAMA_APP_CONVENTIONS.md §6.3` | 15 |
...
| **Inspecteur contextuel (volet droit)** | Trois étages (card / lot / file) : sélection → Infos + preview + actions clonées (cloneActions) + PARAMÈTRES reflétés (initFromSchema : panel read/apply dérivés du schéma, cardSettings via card_gear) ; hydrate aussi les previews de card (hydrateCardPreviews) | `wama/common/static/common/js/wama-inspector.js` | `WAMA_VOLETS.md` | 57 |
| **Inspecteur — champs de détail** | Schéma canonique des infos d'item affichées au volet droit | `wama/common/utils/detail_registry.py` | `INSPECTOR_DETAIL_FIELDS.md` | 61 |
...
| **Socle JS des apps** | Plomberie commune file/cards : csrfFetch, urls, Poller de progression, états vides | `wama/common/static/common/js/wama-app-base.js` | `WAMA_APP_GENERATION_ROUTE.md` | 61 |
...
**Mécanismes déclarés : 129** · domiciles absents : 0 · sans consommateur : 2 · assumés locaux : 17 · modules balayés non rattachés : 42 · **de niveau app sans critère de grille : 38**
...
| `queue_dnd` — Glisser-déposer et sélection multiple de la file | **1** app(s) : imager | `wama/common/static/common/js/wama-queue-dnd.js` |
