...
| 14 | **ingest média** (`source_url`→fichier local) non déclaré : card URL câblée à la main (`show_url`) au lieu d'être dérivée d'une capacité ; download réécrit par app (describer `_download_from_source_url`, transcriber `_ensure_local_audio_from_source_url` — le transcriber **crashait** sans). Manque : capacité F2 `accepts_url`/`accepts_local_path` (→ génère la card) + sous-facette F5 `ingest:{source_field,target_field,mode:media\|audio\|smart}` + mécanisme commun `ensure_local_input(instance)`. Primitives déjà communes : `url_ingest.fetch_url_content`, `video_utils.upload_media_from_url`/`download_youtube_audio`. | F2/F5 | manifeste + réinvention |
