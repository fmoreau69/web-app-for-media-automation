...
| **D11** | **`WamaImport` n'est chargé par AUCUNE des 10 apps en place** — `_app_scripts.html` n'est inclus que par les gabarits GÉNÉRÉS (mesuré 05/09 sur imager, avatarizer, converter : `typeof WamaImport === 'undefined'`). La « voie d'import commune » est donc, aujourd'hui, la voie des seules jumelles ; chaque app en place garde son `handleFiles` | portage | 🔄 **3/10 le 07/09 : transcriber (13/13), converter (13/14 + skip anti-bouclage), describer (12/12) câblés**, critère `import_front` écrit — détail `WAMA_APP_GENERATION_ROUTE §Portage F2` ; suite = synthesizer → enhancer-image → … (ordre §8.6 du handoff) ; **tout helper que l'explorateur (global) doit atteindre va dans `wama-app-base.js`, jamais dans `wama-import.js`** | `common/_app_scripts.html:47` |
