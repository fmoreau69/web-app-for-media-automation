...
| **D11** | **`WamaImport` n'est chargé par AUCUNE des 10 apps en place** — `_app_scripts.html` n'est inclus que par les gabarits GÉNÉRÉS (mesuré 05/09 sur imager, avatarizer, converter : `typeof WamaImport === 'undefined'`). La « voie d'import commune » est donc, aujourd'hui, la voie des seules jumelles ; chaque app en place garde son `handleFiles` | portage | 🔄 **6/10 le 07/09 : transcriber (13/13), converter (13/14 + skip anti-bouclage), describer (12/12), synthesizer (12/13 + skip « pas d'URL »), enhancer-image (11/12 + skip anti-bouclage), reader (11/12 + skip « pas d'URL ») câblés**, critère `import_front` écrit — détail `WAMA_APP_GENERATION_ROUTE §Portage F2` (dont le 500 de consolidation sur FormData, corrigé dans 6 vues) ; suite = anonymizer → composer → enhancer-audio → imager/avatarizer (attache) ; **tout helper que l'explorateur (global) doit atteindre va dans `wama-app-base.js`, jamais dans `wama-import.js`** | `common/_app_scripts.html:47` |
