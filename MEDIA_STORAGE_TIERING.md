...
| **D11** | **`WamaImport` n'est chargé par AUCUNE des 10 apps en place** — `_app_scripts.html` n'est inclus que par les gabarits GÉNÉRÉS (mesuré 05/09 sur imager, avatarizer, converter : `typeof WamaImport === 'undefined'`). La « voie d'import commune » est donc, aujourd'hui, la voie des seules jumelles ; chaque app en place garde son `handleFiles` | portage | 🔄 **2/10 le 07/09 : transcriber (13/13) puis converter (13/14 + skip anti-bouclage) câblés**, critère `import_front` écrit — détail `WAMA_APP_GENERATION_ROUTE §Portage F2` ; suite = describer → synthesizer → … (ordre §8.6 du handoff) ; **tout helper que l'explorateur (global) doit atteindre va dans `wama-app-base.js`, jamais dans `wama-import.js`** | `common/_app_scripts.html:47` |
