...
> - `wama/common/backends/vendor/codeformer` (ex-submodule sous `avatarizer`, déplacé le 2026-09-07) : « contains modified content » **pré-existant**,
...
| **08/09 — LOT VIDÉO à l'imager** (Fabien : « j'ai prévu de pouvoir faire des batchs vidéos, notamment depuis le studio ») : `handle_file2img` écrivait `txt2img` / `domain='image'` EN DUR ; il lit désormais le domaine DÉCLARÉ (`txt2vid`, lot vidéo, durée/fps/résolution) ; la card vidéo déclare sa barre (`batch_bid='vidBatch'`, paramètre ajouté à `_new_item_card`) et sa 2ᵉ instance de brique batch ; le studio postera `domain='video'` au même endpoint | tests `imager/tests.py` (module VIDE jusque-là : vidéo → `txt2vid` + lot video + réglages ; sans déclaration → image) ; sonde : barre `vidBatch` (2), barre image fermée, « Ajouter » → lot video de 2 `txt2vid` ; 66 tests OK ; famille imager 7/12 + 5 skips déclarés |
