...
<div class="modal-dialog modal-dialog-centered">
  <div class="modal-content bg-dark border-secondary text-white">
    ...
    async function showBatchBar(file) {
        _pendingBatchFile = file;
        const bar = document.getElementById('batchDetectBar');
        const preview = document.getElementById('batchDetectPreview');
        const countSpan = document.getElementById('batchDetectedCount');
        if (!bar) return;

        // Quick preview: parse server-side
        const fd = new FormData();
        fd.append('batch_file', file);
        try {
            const r = await csrfFetch(urls.batchPreview, { method: 'POST', body: fd });
            const data = await r.json();
            if (data.error) { alert(data.error); return; }

            countSpan.textContent = data.count;
            document.getElementById('batchCreateCount').textContent = data.count;
            bar.style.display = '';
            ...
            tbody.innerHTML = (data.items || []).map(it =>
                `<tr><td class="text-truncate" style="max-width:200px;" title="${escapeHtml(it.path)}">${escapeHtml(it.filename || it.path)}</td></tr>`
            ).join('');

            // Warnings
            const warnDiv = document.getElementById('batchDetectWarnings');
            ...
            console.error('[Reader] batch preview error:', e);
        }
    }
    ...
    async function createBatch(andStart) {
        if (!_pendingBatchFile) return;
        const progress = document.getElementById('batchCreateProgress');
        if (progress) progress.style.display = '';

        const fd = new FormData();
        fd.append('batch_file', _pendingBatchFile);
        fd.append('backend',       document.getElementById('backendSelect')?.value || 'auto');
        fd.append('mode',          document.getElementById('modeSelect')?.value || 'auto');
        fd.append('output_format', document.getElementById('outputFormatSelect')?.value || 'txt');
        fd.append('language',      document.getElementById('languageInput')?.value.trim() || '');
        ...
        try {
            const r = await csrfFetch(urls.batchCreate, { method: 'POST', body: fd });
            const data = await r.json();
            if (data.error) { alert(data.error); return; }

            hideBatchBar();
            // Reload page to display batch group
            if (andStart) {
                // Start batch then reload
                await csrfFetch(urlForBatch('batchStart', data.batch_id), { method: 'POST' });
            }
            window.location.reload();
        } catch (e) {
            console.error('[Reader] batch create error:', e);
        } finally {
    ...
    function initBatchBar() {
        document.getElementById('batchPreviewBtn')?.addEventListener('click', () => {
            const p = document.getElementById('batchDetectPreview');
            if (p) p.style.display = p.style.display === 'none' ? '' : 'none';
        });
        document.getElementById('batchCancelBar')?.addEventListener('click', hideBatchBar);
        document.getElementById('batchCreateOnlyBtn')?.addEventListener('click', () => createBatch(false));
        document.getElementById('batchCreateAndStartBtn')?.addEventListener('click', () => createBatch(true));
    }
    ...
    function uploadFilesWithBatch(files) {
        if (!files || !files.length) return;
        const arr = Array.from(files);
        // If a single batch file is dropped, show the batch bar
        if (arr.length === 1 && isBatchFile(arr[0])) {
            showBatchBar(arr[0]);
            return;
        }
        uploadFiles(files);
