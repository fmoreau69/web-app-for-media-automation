document.addEventListener('DOMContentLoaded', function () {
  ...
  async function handleFiles(files) {
    for (const file of files) {
      ...
      if (['txt', 'csv', 'md'].includes(ext)) {
        // Treat as potential batch file
        await handleFilesWithDetect(file);
      } else {
  ...
  // ── Batch detect bar (image/video only) ────────────────────────────────
  let _batchFile = null;
  let _batchItems = [];
  ...
  function _populateBatchPreview(data) {
    ...
    (data.items || []).forEach(item => {
      ...
      tr.innerHTML = `<td style="word-break:break-all;">${item.path}</td>` +
        `<td><span class="badge bg-secondary">${item.detected_type || 'media'}</span></td>`;
  ...
  async function _doBatchImport(autoStart) {
    ...
    const formData = new FormData();
    formData.append('batch_file', _batchFile);
    // Pass current default settings
    const aiModel = document.getElementById('defaultAiModel');
    const denoise = document.getElementById('defaultDenoise');
    const blendFactor = document.getElementById('defaultBlendFactor');
    if (aiModel) formData.append('ai_model', aiModel.value);
    if (denoise) formData.append('denoise', denoise.checked ? 'true' : 'false');
    if (blendFactor) formData.append('blend_factor', blendFactor.value);
    ...
    try {
      ...
      _hideBatchBar();
      const msg = `Batch créé (${data.total} éléments)` + (autoStart ? ' — traitement lancé' : '');
      if (window.FileManager && window.FileManager.showToast) {
        window.FileManager.showToast(msg, 'success');
      }
